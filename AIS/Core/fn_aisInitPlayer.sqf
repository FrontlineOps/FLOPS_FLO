...
waitUntil {(count (allMapMarkers select {markerColor _x == "Color1_FD_F"}) > 0)};
...
if (local _player) then {
	...
	[_player] spawn {
		...
		_mrkrs = allMapMarkers select {markerColor _x == "Color1_FD_F"};
        _RVmrkr = _mrkrs select 0;
		if (markerText _RVmrkr == "Activate") then {
